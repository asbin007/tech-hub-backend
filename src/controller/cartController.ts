...
class CartController {
  ...
  if (!validSizes.includes(size)) {
    return res.status(400).json({ message: `Invalid size selected: ${size}` });
  }
  if (!validColors.includes(color)) {
    return res.status(400).json({ message: `Invalid color selected: ${color}` });
  }
  ...
  if (userKoCartExist) {
    ...
    return res.status(200).json({ message: "Cart updated", cart: userKoCartExist });
  }
  ...
  async getCart(req: IAuth, res: Response): Promise<void> {
    ...
    const carts = await Cart.findAll({
      ...
      include: [{ model: Product,
        attributes:['image','price','name','id']
